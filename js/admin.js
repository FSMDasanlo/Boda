document.addEventListener("DOMContentLoaded", () => {
  ...
  const challengesAdminSection = document.getElementById(
    "challenges-admin-section",
  );
  const closeChallengesAdminBtn = document.getElementById(
    "close-challenges-admin-btn",
  );
  ...
  skippedChallengesBtn.addEventListener("click", () =>
    loadAndShowSkippedChallenges(),
  );
  ...
  const renderGallery = () => {
    ...
    const filteredMemories = allMemories.filter((item) => {
      const matchesSearch = item.data.guestName
        .toLowerCase()
        .includes(searchTerm);
      const matchesFilter = showOnlyChallenges
        ? item.data.isChallengeProof
        : true;
    ...
    if (filteredMemories.length === 0) {
      gallery.innerHTML =
        '<p style="text-align: center; width: 100%;">No se encontraron recuerdos.</p>';
    ...
    memoriesToShow.forEach(({ id, data }) => {
      ...
      if (imageUrls && imageUrls.length > 0) {
        ...
        const clickAction = data.isChallengeProof
          ? ""
          : `onclick="window.open(this.src, '_blank')"`;
        const dataAttr = data.isChallengeProof
          ? `data-challenge="${escapeHtml(data.challenge || "")}"`
          : "";
      ...
      card
        ...
        .addEventListener("click", () => deleteMemory(id, data));
      ...
      .onSnapshot((snapshot) => {
        if (snapshot.empty) {
          challengesList.innerHTML =
            "<p>No hay retos definidos. Puedes añadirlos desde el formulario de abajo.</p>";
          return;
        }
        challengesList.innerHTML = "";
        snapshot.forEach((doc) => {
          ...
          const completedBy = allMemories
            .filter(
              (m) =>
                m.data.isChallengeProof && m.data.challenge === combinedText,
            )
          ...
          item
            ...
            .addEventListener("click", () => deleteChallenge(challengeId));
          item
            .querySelector(".edit-challenge-btn")
            .addEventListener("click", (e) =>
              editChallenge(e.target, challengeId, challenge),
            );
  ...
  const saveChallenge = async (id, item) => {
    ...
    try {
      await db
        ...
        .doc(id)
        .update({ text_es: newTextEs, text_en: newTextEn });
  ...
  const createSkippedChallengesSection = () => {
    if (document.getElementById("skipped-challenges-section"))
      return document.getElementById("skipped-challenges-section");
    ...
    document
      .getElementById("close-skipped-challenges-btn")
  ...
  const loadAndShowSkippedChallenges = async () => {
    ...
    const listContainer = skippedChallengesSection.querySelector(
      "#skipped-challenges-list",
    );
    listContainer.innerHTML = "<p>Cargando...</p>";

    try {
      const snapshot = await db
        .collection("skipped_challenges")
        .orderBy("skippedAt", "desc")
        .get();

      if (snapshot.empty) {
        listContainer.innerHTML =
          '<p style="text-align:center; padding: 20px 0;">¡Nadie se ha rajado todavía! ¡Qué valientes!</p>';
        return;
      }
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
        const date = data.skippedAt
          ? new Date(data.skippedAt.seconds * 1000).toLocaleString()
          : "Fecha desconocida";
  ...
  downloadBtn.addEventListener("click", () => {
    ...
    const sortedMemories = [...allMemories].sort((a, b) =>
      a.data.guestName.localeCompare(b.data.guestName),
    );
  ...
  const deleteMemory = async (docId, data) => {
    if (
      !confirm(
        "¿Estás seguro de que quieres borrar este recuerdo permanentemente?",
      )
    )
      return;
      ...
      if (filesToDelete.length > 0) {
        const deletePromises = filesToDelete.map((url) =>
          storage
            .refFromURL(url)
            .delete()
            .catch((e) => console.warn("Archivo ya borrado o inaccesible", e)),
        );
